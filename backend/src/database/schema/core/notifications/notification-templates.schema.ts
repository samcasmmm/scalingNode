...
import { type AnyPgColumn, bigint, pgTable, text, uniqueIndex, varchar } from 'drizzle-orm/pg-core';
...
export const notificationTemplatesTable = pgTable(
   ...
   (t) => [
      uniqueIndex('notification_templates_tenant_key_channel_idx').on(t.tenantId, t.key, t.channel),
   ]
);

export const notificationTemplatesRelations = relations(notificationTemplatesTable, ({ one }) => ({
