...
export const permissionGroupItemsTable = pgTable(
  'permission_group_items',
  {
    permissionGroupId: bigint('permission_group_id', { mode: 'number' })
      .notNull()
      .references((): AnyPgColumn => permissionGroupsTable.id, {
        onDelete: 'cascade',
      }),
  ...
  (t) => [primaryKey({ columns: [t.permissionGroupId, t.permissionId] })],
);
...
export type NewPermissionGroupItem =
  typeof permissionGroupItemsTable.$inferInsert;
