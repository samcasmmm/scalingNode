...
import { type AnyPgColumn, bigint, pgTable, text, timestamp, varchar } from 'drizzle-orm/pg-core';
...
export const temporaryAccessTable = pgTable('temporary_access', {
   ...
   grantedByUserId: bigint('granted_by_user_id', { mode: 'number' }).references((): AnyPgColumn => usersTable.id, {
      onDelete: 'set null',
   }),
   reason: text('reason'),
...
export const temporaryAccessRelations = relations(temporaryAccessTable, ({ one }) => ({
