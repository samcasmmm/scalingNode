...
import { type AnyPgColumn, bigint, index, pgTable, text, varchar } from 'drizzle-orm/pg-core';
import { auditActorColumns, baseColumns, flagColumn, versionColumn } from '@/database/schema/core/_shared.columns.js';
import { tenantsTable } from '@/database/schema/core/tenants/tenants.schema.js';
import { organizationsTable } from '@/database/schema/core/tenants/organizations.schema.js';
import { usersTable } from '@/database/schema/core/users/users.schema.js';
import { teamMembersTable } from '@/database/schema/core/users/team-members.schema.js';
...
export const teamsTable = pgTable(
   'teams',
   {
      ...
      leadUserId: bigint('lead_user_id', { mode: 'number' }).references((): AnyPgColumn => usersTable.id, {
         onDelete: 'set null',
      }),
