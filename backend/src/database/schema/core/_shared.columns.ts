import { AnyPgColumn, bigint, boolean, text, timestamp } from 'drizzle-orm/pg-core';

/** Standard primary key for every table — bigint identity, matches existing schemas (tenants, users, roles, ...). */
export const idColumn = () => bigint('id', { mode: 'number' }).primaryKey().generatedAlwaysAsIdentity();
 ...
 */
export const versionColumn = () => bigint('version', { mode: 'number' }).default(1).notNull();

/** Generic boolean flag column with a configurable name, e.g. `is_active`, `is_enabled`. */
export const flagColumn = (name = 'is_active', defaultValue = true) => boolean(name).default(defaultValue).notNull();
