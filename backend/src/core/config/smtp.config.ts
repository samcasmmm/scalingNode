...
export const getTransporter = (): Transporter => {
   if (!isConfigured) {
      throw new Error('[mail] SMTP is not configured — set SMTP_HOST, SMTP_USER, SMTP_PASSWORD');
   }
   if (!transporter) {
...
export const sendMail = async (options: SendMailOptions): Promise<void> => {
   ...
   try {
      ...
      console.error('[mail] Failed to send email', { to: options.to, subject: options.subject, err });
      throw err;
   }
};
...
if (!environment.PRODUCTION && !isConfigured) {
   console.warn('[mail] SMTP credentials missing — email sending is disabled in this environment');
}
