...
const scheduleReconnect = (): void => {
  if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
    console.error(
      `[rabbitmq] Max reconnect attempts (${MAX_RECONNECT_ATTEMPTS}) reached, giving up`,
    );
    return;
  }
  reconnectAttempts += 1;
  ...
  console.log(
    `[rabbitmq] Reconnecting in ${delay}ms (attempt ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})`,
  );
  setTimeout(() => {
